...
export async function renderSettings(guildId, readOnly = false) {
  const settings = Settings.get(guildId);
  
  const embed = new EmbedBuilder()
    .setTitle("Timo's Handpicked Deals - Settings")
    .setColor(0x0099ff)
    .setDescription(readOnly ? "View-only mode: You need Manage Guild permissions to change settings." : "Configure how the bot behaves in this server.")
    .addFields(
  ...
  // Row 1: Toggles (Moved Reset here to make space)
  ...
  const row5 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('edit_allowlist').setLabel('Allowlist').setStyle(ButtonStyle.Secondary).setDisabled(readOnly),
    new ButtonBuilder().setCustomId('edit_blocklist').setLabel('Blocklist').setStyle(ButtonStyle.Secondary).setDisabled(readOnly),
    new ButtonBuilder().setCustomId('edit_mintemp').setLabel('Min Temp').setStyle(ButtonStyle.Secondary).setDisabled(readOnly),
    new ButtonBuilder().setCustomId('post_test_mydealz').setLabel('Test MyDealz').setStyle(ButtonStyle.Primary).setDisabled(readOnly),
    new ButtonBuilder().setCustomId('post_test_hotukdeals').setLabel('Test HotUKDeals').setStyle(ButtonStyle.Primary).setDisabled(readOnly),
  );
  ...
  if (interaction.isButton()) {
    switch (interaction.customId) {
      ...
      case 'reset_settings':
        ...
        break;
      case 'post_test_mydealz':
        await handleTestPost(interaction, 'mydealz', settings);
        return; 
      case 'post_test_hotukdeals':
        await handleTestPost(interaction, 'hotukdeals', settings);
        return;
...
async function handleTestPost(interaction, source, settings) {
  await interaction.deferReply({ ephemeral: true });
  try {
    const url = FEED_URLS[source];
    const deals = await fetchFeed(url, source, settings);
    
    if (deals.length === 0) {
      await interaction.editReply({ content: `No deals found from ${source} (checked ${url}). Check your filters or the feed URL.` });
      return;
    }

    const deal = deals[0]; // Take the first one
    const embed = createDealEmbed(deal, 'normal');
    
    await interaction.editReply({ 
      content: `Here is the latest deal from **${source}** (fetched live):`, 
      embeds: [embed] 
    });
  } catch (error) {
    console.error('Test post error:', error);
    await interaction.editReply({ content: `Failed to fetch test deal from ${source}: ${error.message}` });
  }
}
